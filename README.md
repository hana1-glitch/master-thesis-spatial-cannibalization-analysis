...
## 🚀 Key Methodology
### 1. Data Engineering
* 서울시 업종별 5개년 공공데이터를 기반으로 공간 데이터 집계 및 특성 추출(Feature Engineering). 특히 Polars 라이브러리를 활용한 고속 데이터 처리를 수행
### 2. Spatial Analysis
* 공간 자기상관 지수(Moran's I, LISA) 분석을 통한 업종별 상권 밀집도 및 클러스터링 패턴 평가
#### 1) Global Moran's I: 공간적 인접성에 따른 상권 활성도의 자기상관성을 측정하여 전체적인 상권 밀집 패턴을 파악
...
#### 2) LISA (Local Indicators of Spatial Association): 행정동 단위의 국소적 공간 자기상관성을 분석하여 상권의 유형을 4가지 클러스터로 분류
* HH (High-High): 상권 활성도가 높고 인접 지역과의 시너지가 발생하는 '핵심 성장 지역'
* LL (Low-Low): 활성도가 낮아 전략적 보완이 필요한 '저활성 클러스터'
* LH/HL: 인접 지역과 다른 독자적 흐름을 보이는 '잠재적 핫스팟 및 고립 지역'

> **이 분석 결과를 바탕으로 '6단계 입지 전략' 모델을 수립했습니다.**

### 3. Threshold Modeling
* 자기잠식이 급격히 발생하는 임계점(Threshold)을 산출하여 최적 입지 전략 모델링 구축
